...
OUTPUT_SQLITE_FILE = os.path.join('..', '..', 'data', 'personal.db')
TABLE_NAME = 'transactions'
SPENDING_OVERRIDES_FILE = os.path.join('..', 'config', 'spending_overrides.json')
...
def get_gspread_client():
    ...
    if not os.path.exists(creds_path):
        ...
        alt_creds_path = '/Volumes/ExtremeSSD/PersonalAgents/PersonalAgents/finances/config/serviceAccountKey.json'
...
def csv_to_sqlite(csv_file_path, db_file_path, table_name='transactions', overwrite=True):
    ...
    Args:
        csv_file_path: Path to CSV file (e.g., INPUT_CSV_FILE which is relative to script)
        db_file_path: Path to SQLite database file (e.g., OUTPUT_SQLITE_FILE which is relative to script)
